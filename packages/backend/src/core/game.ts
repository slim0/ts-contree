...
import { deckOf32Cards } from 'shared/src/eventSchemas/datas/cards'
...
function initGame(
  ...
  return pipe(
    Effect.Do,
    Effect.bind('shuffledDeck', () =>
      Effect.succeed(shuffleArray(deckOf32Cards)),
    ),
    Effect.bind('playerHands', ({ shuffledDeck }) =>
      Effect.succeed([
    ...
    Effect.andThen(({ game, playerHands }) =>
      Effect.forEach(players, (player, index) =>
        Effect.succeed({
